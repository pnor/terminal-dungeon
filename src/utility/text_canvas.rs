...
impl TextCanvas {
    ...
    /// Returns representation of the canvas as a vector of `StyledString`, where each index represents a row
    pub fn as_styled_strings(&self) -> Vec<StyledString> {
        let mut strings = vec![StyledString::default(); self.symbols.len()];

        for i in 0..(self.symbols.len() - 1) {
            for j in 0..(self.symbols[i].len() - 1) {
                let symbol = &self.symbols[i][j];

                strings[i].append(symbol.styled_string());
            }
        }
